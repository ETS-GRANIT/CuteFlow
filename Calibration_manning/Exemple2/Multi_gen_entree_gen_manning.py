...
# Création de la liste des dossiers à accéder avec le modèle 'Case_*'
...
for i, nom_dossier in enumerate(liste_dossiers, start=1):
    ...
    # Extraction des colonnes de données (CellIDS et Manning)
    cellIDs = data.iloc[:,1]
    manning = data.iloc[:,2]
    ...
    with open(outputFilename, 'w') as fid:
        ...
        # Affichage du message d'évolution du script
        print(f'\nFichier {file_path} traité et enregistré sous {outputFilename}.')

# Affichage du message de fin d'exécution de la tâche
print('\nFin du programme!!\n')
