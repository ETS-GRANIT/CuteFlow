...
contains
  ...
  subroutine update_sol_elems
    ...
    do gi=1,nelt
      vdlg(:,gi) = 0.
      do k=1,3
        mgi = connectivite(gi,k)
        if(vdlg_nodes(1,mgi)>tolisec) then
          vdlg(:,gi) = vdlg(:,gi) + vdlg_nodes(:,mgi)
        end if
      end do
      vdlg(:,gi) = vdlg(:,gi)/3.
    end do
  end subroutine update_sol_elems

  subroutine update_sol_nodes
    use precision_kind
    use global_data
    ...
    integer :: gi, mgi, k
    real(fp_kind) :: area_loc

    do gi=1,nnt
      vdlg_nodes(:,gi) = 0.
      do k=1,6
        mgi = vertex_neighbors(k,gi)
        if(mgi > 0) then
          if(vdlg(1,mgi) > tolisec) then
            vdlg_nodes(:,gi) = vdlg_nodes(:,gi) + vdlg(:,mgi)*surf(mgi)
          end if
          area_loc = area_loc + surf(mgi)
        end if
      end do
      if(area_loc > 1e-12) vdlg_nodes(:,gi) = vdlg_nodes(:,gi)/area_loc
    end do
