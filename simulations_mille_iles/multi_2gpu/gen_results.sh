...
for i in `seq 1 $ncas`;do
  ...
  cd ../multi_$i
  if [ -f 0_solution_elements_restart.txt ] && [ $(wc -l 0_solution_elements_restart.txt | cut -d " " -f 1) -ge 1 ]; then
    echo "[+] Cas $i, Fichiers restart trouvés"
    echo "[*] ../base_files/merge_solutions restart solution_elements_restart.txt $ngpu"
    ../base_files/merge_solutions restart solution_elements_restart.txt $ngpu
  else
    echo "[-] Cas $i, Aucun fichier de restart trouvé !!"
  fi

  if [ -f 0_solution_noeuds.txt ] && [ $(wc -l 0_solution_noeuds.txt | cut -d " " -f 1) -ge 1 ]; then
    echo "[+] Cas $i, Fichiers sol2d trouvés"
    echo "[*] ../base_files/merge_solutions noeuds solution_noeuds.txt $ngpu"
    ../base_files/merge_solutions noeuds solution_noeuds.txt $ngpu
  else
